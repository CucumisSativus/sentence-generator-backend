...
import net.cucumbersome.sentenceGenerator.sentenceGenerator.SimpleSentenceGeneratorTest._
import net.cucumbersome.sentenceGenerator.wordGenerator.NextWordGenerator
import org.scalatest.{Matchers, WordSpec}

class SimpleSentenceGeneratorTest extends WordSpec with Matchers {
  "sentence generator" when {
    "generating single sentence" should {
      "generate 5 words sentence if word generator allows it" in {
        val sentenceGenerator: SimpleSentenceGenerator = buildSentenceGenerator
  ...
  private def buildSentenceGenerator = {
    ...
    new SimpleSentenceGenerator(wordsGenerator)
  }
}

object SimpleSentenceGeneratorTest {
